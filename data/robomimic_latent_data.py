...
class RobomimicLatentDataset(torch.utils.data.IterableDataset):
    def __init__(self, hdf5_path, latent_path, obs_keys, rgb_keys, dataset_keys,
        frame_stack, seq_length, hdf5_use_swmr, n_overfit, optimal,
    ):
        super(RobomimicLatentDataset, self).__init__()

        self.hdf5_path = os.path.expanduser(hdf5_path)
        self.hdf5_use_swmr = hdf5_use_swmr
        self._hdf5_file = None
        self.latent_path = os.path.expanduser(latent_path)
        self._latent_file = None
    ...
    def load_demo_info(self):
        ...
        print(f"dataset will load {'all' if self.n_overfit is None else self.n_overfit} trajectories")
        if isinstance(self.n_overfit, omegaconf.listconfig.ListConfig):
            overfit_demos = []
            for demo in self.demos:
                if demo in self.n_overfit:
                    overfit_demos.append(demo)
            self.demos = overfit_demos
            print(f"selected {len(self.demos)} demos")
        elif self.n_overfit is not None:
        ...
        for demo in self.demos:
            for key in self.obs_keys: 
                if key == 'optimal':
                    ...
                    obs = self.latent_file['data'][demo]['latent'][key.replace('latent_', '')][:]
                    dict_list[key].append(obs)
                else:
    ...
    def latent_file(self):
        ...
        This property allows for a lazy hdf5 file open.
        """
        if self._latent_file is None:
            self._latent_file = h5py.File(self.latent_path, 'r', swmr=self.hdf5_use_swmr, libver='latest')
        return self._latent_file
    ...
    def close_and_delete_hdf5_handle(self):
        ...
        if self._latent_file is not None:
            self._latent_file.close()
        self._hdf5_file = None
...
class RobomimicData:
    ...
    def train_dataset(self):
        if self._train_dataset is None:
            ...
            self.ds_kwargs['latent_path'] = self.train_latent_path
            self.ds_kwargs['n_overfit'] = self.train_n_episode_overfit
            self._train_dataset = RobomimicLatentDataset(**self.ds_kwargs)
    ...
    def val_dataset(self):
        if self._val_dataset is None:
            ...
            self.ds_kwargs['latent_path'] = self.eval_latent_path
            self.ds_kwargs['n_overfit'] = self.eval_n_episode_overfit
            self._val_dataset = RobomimicLatentDataset(**self.ds_kwargs)
