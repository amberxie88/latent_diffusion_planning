...
from data.robomimic_latent_data import RobomimicLatentDataset as RobomimicLatentDataset
import torch
import utils.py_utils as py_utils

class RobomimicMixedLatentData(torch.utils.data.IterableDataset):
	"""
	...
	def __init__(self, name, train_paths, eval_paths,
		...
		for idx, (dataset_path, n_overfit) in enumerate(zip(self.train_paths, self.train_n_episode_overfit)):
			...
			self.train_datasets.append(RobomimicLatentDataset(**self.ds_kwargs))
