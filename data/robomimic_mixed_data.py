...
from data.robomimic_data import RobomimicDataset as RobomimicDataset
import torch
import utils.py_utils as py_utils

class RobomimicMixedData(torch.utils.data.IterableDataset):
	"""
	...
	def __init__(self, name, train_paths, eval_paths,
		...
		for idx, (dataset_path, n_overfit) in enumerate(zip(self.train_paths, self.train_n_episode_overfit)):
			...
			self.train_datasets.append(RobomimicDataset(**self.ds_kwargs))
	...
	@property
	def env_meta(self):
		return self.train_datasets[0].env_meta
