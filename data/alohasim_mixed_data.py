...
from data.alohasim_data import ALOHASimDataset as ALOHASimDataset
import torch
import utils.py_utils as py_utils

class ALOHASimMixedData(torch.utils.data.IterableDataset):
	"""
	Load offline ALOHASim data.
	"""
	...
	def __init__(self, name, train_paths, eval_paths,
		...
		for idx, (dataset_path, n_overfit) in enumerate(zip(self.train_paths, self.train_n_episode_overfit)):
			...
			self.train_datasets.append(ALOHASimDataset(**self.ds_kwargs))
	...
	def sample_traj(self, ep_id):
		return self.train_datasets[ep_id % len(self.train_datasets)].sample_traj(ep_id // len(self.train_datasets))
